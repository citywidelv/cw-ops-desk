 ...
 *  * CW Team Directory - staff list for the Ops Hub (first consumer: create-exhibit-a.html).
  * Standalone Google Sheet; ID in ScriptProperties key STAFF_SHEET_ID.
   * staff_setup creates the sheet on first run; team edits the Staff tab directly.
    * Routed from doPost: kinds starting staff_ come here. Passcode checked server side.
     */
                     ...
                     function staffPass_() {
                       try { if (typeof PASSCODE !== 'undefined') return PASSCODE; } catch (e) {}
                         ...
                         function staffOut_(o) {
                           ...
                           function staffSS_() {
  // Router over the team family (Books.gs): CW Team Directory, with the
  // Exhibit A Options tab answered by CW Exhibit A Options.
  return cwRouter_('staff');
}
                                                     ...
                                                     var sh = ss.getSheetByName('Staff');
                                                       if (!sh) {
                                                           sh = ss.insertSheet('Staff');
                                                               sh.getRange(1, 1, 1, STAFF_HEADERS.length).setValues([STAFF_HEADERS]).setFontWeight('bold').setBackground('#D22730').setFontColor('#FFFFFF');
                                                                                                                           ...
                                                                                                                           for (var i = 1; i < v.length; i++) {
                                                                                                                               if (!v[i][0]) continue;
                                                                                                                                   if (String(v[i][5]).toUpperCase() === 'FALSE') continue;
                                                                                                                                       out.push({ name: String(v[i][0]), role: String(v[i][1] || ''), market: String(v[i][2] || ''), phone: String(v[i][3] || ''), email: String(v[i][4] || ''), territory: String(v[i][6] || '') });
                                                                                                                                         }
  ...
  var sh = ss.getSheetByName(STAFF_OPT_TAB);
    if (!sh) {
        sh = ss.insertSheet(STAFF_OPT_TAB);
            sh.getRange(1, 1, 1, 3).setValues([STAFF_OPT_HEADERS]).setFontWeight('bold').setBackground('#D22730').setFontColor('#FFFFFF');
